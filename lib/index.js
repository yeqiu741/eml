...
module.exports = async function(size) {
  ...
  spinner.color = 'green'
  spinner.start()
  const { error, data: { stdout, stderr } } = await execPromise('npm ls -g -dep0')
  if (error) {
  ...
  if (stdout.indexOf('increase-memory-limit') === -1) {
    ...
    const { error, data: { stdout, stderr } } = await execPromise('npm i increase-memory-limit -g')
    if (error) {
      spinner.fail(chalk.red(`执行出错： ${ error }`))
      return
    }
  ...
  if (stdout.indexOf('cross-env') === -1) {
    ...
    const { error, data: { stdout, stderr } } = await execPromise('npm i cross-env -g')
    if (error) {
      spinner.fail(chalk.red(`执行出错： ${ error }`))
      return
    }
  ...
  spinner.start()
  const { error: kerror } = await execPromise(`cross-env LIMIT=${ size } increase-memory-limit`)
  if (kerror) {
  ...
  let [err, data] = await to(fs.readdir(nodeModulesDirPath))
  console.log(err, data)
  if (err) return spinner.error(chalk.red(`node_modules文件夹读取失败，失败信息：${err}`))

  // fs.readdir(path.resolve(cwd, './node_modules/.bin'), (err, files) => {
  //   let count = 0
  //   let tag = 0
  //   let failState = false
  //   if (err) {
  ...
  //   //   let [ err, data ] = await to(fs.readFile(targetPath))
  //   // }
